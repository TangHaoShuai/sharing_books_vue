<template>
	<view>
		...
		<view class="u-page">
			...
			<!-- beg账单 -->
			<view>
				<u-card :show-head="false" v-on:click="btn_updatedetail(item)" v-for="(item,index) in bills">
							...
							<u-col span="9" text-align="center">
								<u-row gutter="5" justify="start">
									...
									<u-col span="2" v-if="item.img">
										<view class="" @tap.stop="getMaxImg(item.img)">
											<u-image width="70rpx" height="70rpx" :src="(bill_img_url+item.img)">
											</u-image>
										</view>
									</u-col>
								</u-row>
	...
	export default {
		...
		methods: {
			getMaxImg(item) {
				...
				mzz.show_img = true
				mzz.content = ''
				mzz.img_src = mzz.bill_img_url + item
			},
			//刷新
			refresh() {
				let mzz = this
