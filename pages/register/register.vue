<template>
	<view>
		...
		<u-form :model="form" ref="uForm" class="form">
			...
			<u-form-item label="年龄" prop="age" labelWidth="160">
				<u-input v-model="form.age" class="form-item" placeholder="请输入年龄" />
			</u-form-item>
		...
		methods: {
			...
			submit() {
				this.$refs.uForm.validate(valid => {
					if (valid) {
						this.$request('user/addUser', this.form, 'POST').then(res => {
							// 打印调用成功回调
							console.log(res)
							if (res) {
								// this.$u.toast('注册成功');
